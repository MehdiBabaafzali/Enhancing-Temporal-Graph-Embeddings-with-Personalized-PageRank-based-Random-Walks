...
from models.modules import MLPClassifierWithAgg
...
if __name__ == "__main__":
    ...
    node_raw_features, edge_raw_features, full_data, train_data, val_data, test_data, eval_metric_name, num_classes,p_matrices = \
    ...
    for run in range(args.num_runs):
        ...
        node_classifier = MLPClassifierWithAgg(input_dim=args.output_dim, output_dim=num_classes, dropout=args.dropout)
        ...
        for epoch in range(args.num_epochs):
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                #                 i_node = batch_src_node_ids[i]  # Get the source node ID
                #                 i_year = batch_node_interact_times[i]  # Get the interaction year/time
                #                 p_value = p_matrices[i_year][i_node]  # Get the PPR vector for the node in that year
                #                 non_grad_tensor = torch.tensor(p_value, dtype=batch_src_node_embeddings.dtype).to(batch_src_node_embeddings.device)

                # # Add the PPR vector to the node's embedding
                #                 batch_src_node_embeddings[i] = batch_src_node_embeddings[i] + non_grad_tensor
                        else:
                            raise ValueError(f"Wrong value for model_name {args.model_name}!")
                    else:
